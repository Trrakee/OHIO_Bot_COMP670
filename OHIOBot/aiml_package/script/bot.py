...
def read_args():
    ...
    g11.add_argument( '--aiml_package', nargs='+', help='Load AIML file(s)' )
...
def main():
    ...
    if args.standard:
        chdir = os.path.join(aiml_package.__path__[0], 'botdata', 'standard')
        kern.bootstrap(learnFiles="startup.xml", commands="load aiml_package b",
                       chdir=chdir)
    elif args.alice:
        chdir = os.path.join(aiml_package.__path__[0], 'botdata', 'alice')
