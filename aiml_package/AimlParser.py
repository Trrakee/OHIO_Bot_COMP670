...
class AimlHandler(ContentHandler):
    ...
    def _startElement(self, name, attr):
        if name == "aiml_package":
            # <aiml_package> tags are only legal in the OutsideAiml state
            if self._state != self._STATE_OutsideAiml:
                raise AimlParserError( "Unexpected <aiml_package> tag "+self._location() )
            ...
            try: self._version = attr["version"]
                ...
                #raise AimlParserError( "Missing 'version' attribute in <aiml_package> tag "+self._location() )
                #print( "WARNING: Missing 'version' attribute in <aiml_package> tag "+self._location() )
            ...
            #       raise AimlParserError( "Missing 'version' attribute(s) in <aiml_package> tag "+self._location() )
    ...
    def _endElement(self, name):
        ...
        """
        if name == "aiml_package":
            # </aiml_package> tags are only legal in the InsideAiml state
            if self._state != self._STATE_InsideAiml:
                raise AimlParserError( "Unexpected </aiml_package> tag "+self._location() )
            self._state = self._STATE_OutsideAiml
